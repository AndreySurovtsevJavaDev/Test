...
// Двойным равенством сравниваются примитивы (т.к сравниваются ссылки, а не сами значения), а объекты (ссылочные типы) сравниваем с помощью equals.
// equals определён в классе Object и настраивается под каждый объект с помощью переопределения.
//--------------------------------------------------------------------------------------------------------------------
...
// Stream API - По сути это набор готовых методов, которые упрощают написание кода для реализации различных операций: фильтрации, сортировки и других манипуляции с данными
// Методы (Они же - операторы) стримов делятся на 2 категории:
//  - конвеерные. Позволяют осуществлять какие-то действия со стримом. (что-то изменять в данных / сортировать / фильтровать).
//  - терминальные. Обрабатывают элементы и завершают работу стрима

// вот лямбда-выражения, в стримах, похоже и используются и выглядит всё просто, но как отдельно ими пользоваться - х.з
//--------------------------------------------------------------------------------------------------------------------
...
//(несколько потоков не смогут обратиться к одной переменной, без соответствующей синхронизации).
//--------------------------------------------------------------------------------------------------------------------
...
// Конструкция try-with-resources позволяет вызывать метод close у переменных объявленных в блоке try автоматически
// Пример:
// Когда мы работаем с входящими потоками, например, читаем файлы, есть вероятность забыть закрыть поток и он останется висеть, тем самым заблокирует любую другую работу с файлом..
// Можно воспользоваться и стандартной конструкцией try-catch, но она очень громоздкая и выполнение блока finaly (в котором мы закрываем поток) не всегда корректно
//т.к в процессе может, например, быть выброшено исключение, а поток будет при любых закрыт.
